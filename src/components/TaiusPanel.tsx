...
import { validateHeight, validateG } from '../lib/validate';

export default function TaiusPanel({ project, onUpdate }: {
  project: Project;
  ...
  // Local string state for inputs (numbers shown as strings while typing)
  ...
  useEffect(() => {
    setHeightStr(taius.height > 0 ? String(taius.height) : '');
    setGStr(taius.g > 0 ? String(taius.g) : '');
  }, [project.id]);

  const commit = (field: 'species' | 'height' | 'g', value: string | number) => {
    const updated = { ...taius, [field]: typeof value === 'string' ? parseFloat(value) || 0 : value };
    onUpdate({ taius: updated });
  };

  const handleSave = () => {
    onUpdate({ taius: { ...taius, height: parseFloat(heightStr) || 0, g: parseFloat(gStr) || 0 } });
    flashSaved();
  };
    ...
    <div className="space-y-8">
      <section className="card p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end mb-6">
          <div className="space-y-1">
            <label className="label-cap" htmlFor="taius-species">Puuliik (liigiomane Gn)</label>
          ...
          <div className="space-y-1">
            <label className="label-cap" htmlFor="taius-height">Puistu kõrgus H (m)</label>
            <input id="taius-height" type="number" className={`input-field w-full ${validateHeight(heightStr) && heightStr ? 'border-rose-400' : ''}`} value={heightStr}
              onChange={e => setHeightStr(e.target.value)}
              onBlur={() => commit('height', heightStr)} />
          </div>
          <div className="space-y-1">
            <label className="label-cap" htmlFor="taius-g">Mõõdetud G (m²/ha)</label>
            <input id="taius-g" type="number" className={`input-field w-full ${validateG(gStr) && gStr ? 'border-rose-400' : ''}`} value={gStr}
      ...
      {taiusVal > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <ResultBox
            label="Täius (T = G / Gn × 100)"
            value={taiusVal.toFixed(1) + ' %'}
            type={taiusVal < 30 ? 'danger' : taiusVal < 60 ? 'warn' : 'success'}
            subtext={taiusVal < 30 ? 'Uuendamiskohustus (alla 30%)' : 'Tihedus on normis.'}
          />
        </div>
      )}
      ...
      <section className="card">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => setShowRef(!showRef)}>
          <SectionTitle>Standardtabel Gn (m²/ha) — 100% täius</SectionTitle>
