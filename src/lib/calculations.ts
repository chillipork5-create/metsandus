import { LISA2_DATA, LISA2_STEP, KANDU_CONV_COEFFS, STD_TABLE, MAHT_KORDAJAD, VORMIARV_K, NORM_TAGAVARA_K, TAGAVARA2_K, LISA3_RATES } from './constants';
...
export function getGn(species: string, height: number): number {
  ...
  for (let i = 0; i < heights.length - 1; i++) {
    if (height >= heights[i] && height <= heights[i + 1]) { lo = heights[i]; hi = heights[i + 1]; break; }
  }
