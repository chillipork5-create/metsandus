      ...
      <section className="card p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end mb-6">
          <div className="space-y-1">
            <label className="label-cap" htmlFor="tagavara-species">Puuliik</label>
          ...
          <div className="space-y-1">
            <label className="label-cap" htmlFor="tagavara-g">G (m²/ha)</label>
            <input id="tagavara-g" type="number" className={`input-field w-full ${validateG(gStr) && gStr ? 'border-rose-400' : ''}`} value={gStr}
              onChange={e => setGStr(e.target.value)}
              onBlur={() => commit('g', gStr)} />
          </div>
          <div className="space-y-1">
            <label className="label-cap" htmlFor="tagavara-h">Keskmine kõrgus H (m)</label>
            <input id="tagavara-h" type="number" className={`input-field w-full ${validateHeight(hStr) && hStr ? 'border-rose-400' : ''}`} value={hStr}
