...
import { validateDiameter, validateCount, validateAge, validateArea, validatePerimeter, validateG } from '../lib/validate';
    ...
    <div className="card overflow-hidden border border-slate-200 shadow-sm">
      ...
      {isOpen && (
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
                  ...
                  return (
                    <React.Fragment key={species}>
                      {rates.map((row, idx) => {
                        ...
                        const displayAge = row[0] === 999 ? `üle ${previousAge}` : `${previousAge + 1} kuni ${row[0]}`;
                        return (
          ...
          <p className="mt-3 p-3 bg-indigo-50/70 rounded-lg text-[10px] leading-relaxed text-indigo-900 border border-indigo-100/50">
            Kahjumäär eurodes iga rinnaspindala (m²/ha) puudujääva osa kohta. Vanuse 0–20 ja 101+ korral kahju ei arvestata (—).
          </p>
        </div>
      )}
    </div>
...
// --- Main panel ---
export default function Lisa3Panel({ project, onUpdate }: {
  project: Project;
  ...
  const [treeErrors, setTreeErrors] = useState<{ diam?: string; arv?: string }>({});
  ...
  const handleMapApply = (areaHa: number, perimeter: number) => {
    setAreaStr(String(areaHa));
    setPerimStr(String(perimeter));
  ...
  const handleSave = () => {
    onUpdate({
      lisa3: {
        ...lisa3,
        vanus: parseFloat(vanusStr) || 0,
        alammaar: parseFloat(alammaarStr) || 0,
        tegelik: parseFloat(tegelikStr) || 0,
        measuredArea: parseFloat(areaStr) || 0,
        perimeter: parseFloat(perimStr) || 0,
      },
    });
    flashSaved();
  };
  ...
  const handleAddTree = () => {
    const diamErr = validateDiameter(treeData.diam);
    const arvErr = validateCount(treeData.arv);
    if (diamErr || arvErr) {
      setTreeErrors({ diam: diamErr ?? undefined, arv: arvErr ?? undefined });
      return;
    }
    setTreeErrors({});
    const rows = [...lisa3.rows, { id: generateId(), liik: treeData.liik, grupp: 1, diam: parseFloat(treeData.diam), arv: parseInt(treeData.arv) }];
    onUpdate({ lisa3: { ...lisa3, rows } });
    setTreeData({ ...treeData, diam: '', arv: '' });
  };

  const pindala = useMemo(() =>
    calcKorrigeeritudPindala(parseFloat(perimStr) || 0, parseFloat(areaStr) || 0),
    [perimStr, areaStr]);

  // FIX: use canonical calcG from calculations.ts
  const calculatedG = useMemo(() => {
    if (!pindala || pindala <= 0 || lisa3.rows.length === 0) return 0;
    const totalG = lisa3.rows.reduce((acc, row) => acc + calcG(row.diam, row.arv), 0);
  ...
  const result = useMemo(() => {
    const v = parseFloat(vanusStr) || 0;
    const am = parseFloat(alammaarStr) || 0;
    const k = lisa3.kordaja;
    ...
    return { shortfall, kahju: shortfall * rate * k * pindala, rate, status: 'violation' as const };
  ...
  return (
    <div className="space-y-6">
      <MapPolygonSelector
        ...
        initialArea={areaStr}
        initialPerimeter={perimStr}
        initialPoints={lisa3.coords}
        onPointsChange={pts => onUpdate({ lisa3: { ...lisa3, coords: pts } })}
      />
        ...
        <div className="space-y-6">
          ...
          <section className="card p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 items-end">
              <div className="space-y-1">
                <label className="label-cap" htmlFor="lisa3-tree-liik">Puuliik</label>
              ...
              <div className="space-y-1">
                <label className="label-cap" htmlFor="lisa3-tree-diam">Diameeter d₁.₃ (cm)</label>
                <input id="lisa3-tree-diam" type="number" className="input-field w-full" value={treeData.diam} onChange={e => setTreeData({ ...treeData, diam: e.target.value })} placeholder="24" />
              </div>
              <div className="space-y-1">
                <label className="label-cap" htmlFor="lisa3-tree-arv">Arv (tk)</label>
                <div className="flex gap-2">
                  <input type="number" className="input-field w-full" value={treeData.arv}
                    onChange={e => setTreeData({ ...treeData, arv: e.target.value })}
                    onKeyDown={e => e.key === 'Enter' && handleAddTree()} placeholder="10" />
                      ...
                      return (
                        <tr key={r.id} className="hover:bg-slate-50 group">
                          ...
                          <td className="px-4 py-2 text-right">
                            <button
                              onClick={() => onUpdate({ lisa3: { ...lisa3, rows: lisa3.rows.filter(x => x.id !== r.id) } })}
                              className="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity"
                              aria-label="Eemalda puu"
                            >
          ...
          <section className="card p-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="label-cap" htmlFor="lisa3-puuliik">Enamuspuuliik</label>
              ...
              <div className="space-y-1">
                <label className="label-cap" htmlFor="lisa3-vanus">Puistu vanus (a)</label>
                <input id="lisa3-vanus" type="number" className="input-field w-full" value={vanusStr}
                  onChange={e => setVanusStr(e.target.value)}
                  onBlur={() => commitField('vanus', vanusStr)} />
                {vanusStr && validateAge(vanusStr) && (
                  <p className="text-[10px] text-rose-600 font-medium mt-1" role="alert">{validateAge(vanusStr)}</p>
                )}
                {ageZeroRate && !validateAge(vanusStr) && (
              ...
              <div className="space-y-1">
                <label className="label-cap" htmlFor="lisa3-alammaar">Alammäär G (m²/ha)</label>
                <input id="lisa3-alammaar" type="number" className="input-field w-full" value={alammaarStr}
                  onChange={e => setAlammaarStr(e.target.value)}
                  onBlur={() => commitField('alammaar', alammaarStr)} />
              </div>
              <div className="space-y-1">
                <label className="label-cap">Tegelik G pärast raiet</label>
                <input
                  type="number"
                  className={`input-field w-full ${lisa3.rows.length > 0 ? 'bg-indigo-50 text-indigo-900 font-bold border-indigo-200' : ''}`}
                  readOnly={lisa3.rows.length > 0}
                  value={lisa3.rows.length > 0 ? calculatedG.toFixed(2) : tegelikStr}
                  onChange={e => setTegelikStr(e.target.value)}
                  onBlur={() => commitField('tegelik', tegelikStr)}
                />
          ...
          <section className="card p-8">
            ...
            <div className="grid grid-cols-2 gap-4 items-end">
              <div className="space-y-1">
                ...
                <input type="number" className="input-field w-full" value={areaStr}
                  onChange={e => setAreaStr(e.target.value)}
                  onBlur={() => commitField('measuredArea', areaStr)} placeholder="1.20" />
              </div>
              <div className="space-y-1">
                <label className="label-cap">Ümbermõõt (m)</label>
                <input type="number" className="input-field w-full" value={perimStr}
                  onChange={e => setPerimStr(e.target.value)}
                  onBlur={() => commitField('perimeter', perimStr)} placeholder="550" />
              </div>
            </div>
