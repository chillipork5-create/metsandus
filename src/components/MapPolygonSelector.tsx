    ...
    <div className="card border border-slate-200 shadow-md p-6 bg-white overflow-hidden font-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 border-b border-slate-100 pb-4">
        ...
        <div className="flex gap-2">
          <button 
            type="button"
            className={`px-3 py-1 text-[11px] font-bold rounded-lg ${selectedCoordSystem === 'lest97' ? 'bg-indigo-600 text-white shadow-xs' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
          ...
          <button 
            type="button"
            className={`px-3 py-1 text-[11px] font-bold rounded-lg ${selectedCoordSystem === 'wgs84' ? 'bg-indigo-600 text-white shadow-xs' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <div className="absolute left-4 top-4 z-10 flex flex-col gap-2">
              <div className="flex items-center gap-1.5 p-1 bg-slate-800/80 rounded-lg border border-slate-700 backdrop-blur-xs">
                <button
                  ...
                  className={`p-1.5 rounded-md ${activeCursorMode === 'draw' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                >
                  <MousePointer className="w-3.5 h-3.5" />
                </button>
                <button
                  onClick={() => setActiveCursorMode('pan')}
                  className={`p-1.5 rounded-md ${activeCursorMode === 'pan' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                >
              ...
              <div className="flex flex-col gap-1 p-1 bg-slate-800/80 rounded-lg border border-slate-700 backdrop-blur-xs text-white">
                <button 
                  ...
                  className="p-1.5 hover:text-indigo-400 font-bold transition-all"
                >
                  <ZoomIn className="w-3.5 h-3.5" />
                </button>
                <button 
                  onClick={() => setZoomLevel(prev => Math.max(prev * 0.8, 0.1))}
                  className="p-1.5 hover:text-indigo-400 font-bold transition-all border-t border-slate-700"
                >
                  <ZoomOut className="w-3.5 h-3.5" />
                </button>
                <button 
                  onClick={handleFitView}
                  className="p-1.5 hover:text-indigo-400 text-[9px] font-bold transition-all border-t border-slate-700"
                >
        ...
        <div className="md:col-span-4 flex flex-col gap-4">
          <div className="flex flex-col gap-1.5 h-full">
            ...
            <textarea
              className="w-full flex-1 max-h-[220px] min-h-[160px] md:h-auto font-mono text-[10px] p-3 text-slate-700 border border-slate-200 rounded-xl bg-slate-50 hover:bg-slate-50/50 focus:bg-white custom-scrollbar focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 outline-hidden transition-all placeholder:text-slate-400 placeholder:italic leading-relaxed resize-none"
              value={textAreaValue}
