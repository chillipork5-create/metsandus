...
// FIX: no more require() — proper import
export const SaveButton = ({ saved, saving, onClick }: { saved: boolean; saving?: boolean; onClick: () => void }) => (
  ...
  >
    {saved
      ? <><Check className="w-4 h-4" aria-hidden="true" /> Andmed salvestatud!</>
      : <><Save className="w-4 h-4" aria-hidden="true" /> {saving ? 'Salvestab...' : 'Salvesta andmed'}</>
    }
  </button>
);
