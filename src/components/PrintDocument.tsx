...
export default function PrintDocument({ project, onScreen = false }: {
  ...
  const lisa3CalculatedG = (() => {
    if (!lisa3Pindala || lisa3.rows.length === 0) return 0;
    return lisa3.rows.reduce((acc, row) => acc + calcG(row.diam, row.arv), 0) / lisa3Pindala;
  })();
  const lisa3EffectiveG = lisa3.rows.length > 0 ? lisa3CalculatedG : lisa3.tegelik;
  const lisa3Shortfall = lisa3.alammaar - lisa3EffectiveG;
  const lisa3Rate = getLisa3Rate(lisa3.puuliik, lisa3.vanus) || 0;
  const lisa3Total = lisa3Shortfall > 0 && lisa3Pindala > 0
    ? lisa3Shortfall * lisa3Rate * lisa3.kordaja * lisa3Pindala
    : 0;
  const hasLisa3 = lisa3.measuredArea > 0;
  const combinedTotal = (hasLisa2 ? lisa2Total : 0) + (hasLisa3 ? lisa3Total : 0);

  // FIX: coords is now GeoPoint[] directly, no JSON.parse needed
  const points = lisa3.coords ?? [];
  ...
  const bounds = (() => {
    ...
    const dx = Math.max(maxX - minX, 10), dy = Math.max(maxY - minY, 10);
    return { cx: (minX + maxX) / 2, cy: (minY + maxY) / 2, dx: dx * 1.3, dy: dy * 1.3 };
  })();

  const vw = 500, vh = 220;
  const mapScale = Math.min(vw / bounds.dy, vh / bounds.dx);
  const w2s = (wx: number, wy: number) => ({ x: vw / 2 + (wy - bounds.cy) * mapScale, y: vh / 2 - (wx - bounds.cx) * mapScale });
  ...
  return (
    ...
    >
      ...
      <div className="border-b-4 border-slate-900 pb-5 mb-6 flex justify-between items-end">
        <div>
          <div className="text-[10px] font-bold text-indigo-700 uppercase tracking-widest mb-1">Metsaseaduse Kohane Keskkonnakahju Arvutusprotokoll</div>
        ...
        <div className="text-right">
          <div className="text-sm font-bold text-slate-800">{meta.koostaja || 'Koostaja täitmata'}</div>
          <div className="text-xs text-slate-400 mt-0.5">{meta.kuupaev}</div>
        </div>
      </div>
      ...
      {hasLisa2 && (
        <div className="mb-8 avoid-break">
          <h2 className="text-sm font-black text-slate-800 uppercase tracking-widest border-b-2 border-slate-300 pb-1.5 mb-3">
            LISA 2: Kasvavate puude ebaseadusliku raiumise kahjuarvutus (MS § 67 L2)
          </h2>
          <table className="w-full text-xs mb-4 border-collapse">
            <thead>
              <tr className="bg-slate-50 border-y border-slate-200">
                {['Puuliik','Grupp','Diameeter (cm)','Arv (tk)','Hinnamäär (€)','Summa (€)'].map(h => (
                  <th key={h} className="p-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right first:text-left">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {lisa2.rows.map(r => (
                <tr key={r.id}>
                  ...
                  <td className="p-2 text-right font-mono font-bold">{(getLisa2Rate(r.diam, r.grupp) * r.arv * lisa2.kordaja).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t border-slate-300 font-bold bg-slate-50">
                <td colSpan={5} className="p-2 text-right text-slate-500 text-[10px] uppercase tracking-wider">Lisa 2 kahjusumma kokku:</td>
                <td className="p-2 text-right font-mono">{lisa2Total.toFixed(2)} €</td>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      {hasLisa3 && (
        <div className="mb-8 avoid-break">
          <h2 className="text-sm font-black text-slate-800 uppercase tracking-widest border-b-2 border-slate-300 pb-1.5 mb-3">
            LISA 3: Puistu rinnaspindala kahjustamise kahjuarvutus (MS § 67 L3)
          </h2>
          <div className="grid grid-cols-2 gap-6 mb-4">
            <table className="w-full text-xs divide-y divide-slate-100"><tbody>
              ...
              <tr><td className="py-1.5 text-slate-500">Korrigeeritud pindala P:</td><td className="py-1.5 font-mono font-bold text-indigo-700">{lisa3Pindala.toFixed(4)} ha</td></tr>
            </tbody></table>
            <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs space-y-2">
              ...
              <div className="font-mono text-[10px] text-slate-500">Kahju = {lisa3Shortfall > 0 ? lisa3Shortfall.toFixed(2) : '0'} × {lisa3Rate} × {lisa3.kordaja} × {lisa3Pindala.toFixed(3)}</div>
              <div className="p-2 bg-indigo-50 border border-indigo-200 rounded flex justify-between font-bold text-indigo-900">
                <span>Lisa 3 kahju:</span><span className="font-mono">{lisa3Total.toFixed(2)} €</span>
              </div>
            </div>
          </div>
              ...
              <div>
                ...
                <div className="border border-slate-300 rounded-xl overflow-hidden p-2 bg-slate-100">
                  <svg viewBox={`0 0 ${vw} ${vh}`} className="w-full h-[220px]">
                    ...
                    <polygon points={points.map(p => { const s = w2s(p.x, p.y); return `${s.x},${s.y}`; }).join(' ')} fill="rgba(79,70,229,0.08)" stroke="#4f46e5" strokeWidth="2" strokeDasharray="4,2" />
                    {points.map((p, idx) => { const s = w2s(p.x, p.y); return (
                      <g key={p.id}><circle cx={s.x} cy={s.y} r={6} fill="#fff" stroke="#1e1b4b" strokeWidth="1.5" /><text x={s.x} y={s.y+2.5} fontSize="8" fontWeight="bold" fill="#1e1b4b" textAnchor="middle">{idx+1}</text></g>
                    ); })}
                  </svg>
                </div>
              </div>
              <div>
                <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mb-2">Piiri koordinaadid (L-EST'97 / GPS)</div>
                <table className="w-full text-[9px] border-collapse border border-slate-300 rounded-xl overflow-hidden">
                  <thead><tr className="bg-slate-100 border-b border-slate-300 font-bold text-slate-700">
                    <th className="p-2 text-center w-8">Tipp</th>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {points.map((p, idx) => (
                      <tr key={p.id}>
                        ...
                        <td className="p-1.5 font-mono text-slate-500">{p.lat.toFixed(5)}° N, {p.lon.toFixed(5)}° E</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div className="mt-8 border-t-4 border-slate-900 pt-6 avoid-break">
        <div className="grid grid-cols-2 gap-6 items-end">
          ...
          <table className="w-full text-xs font-medium ml-auto max-w-[325px]"><tbody>
            ...
            {hasLisa3 && <tr><td className="py-1 text-slate-500">Lisa 3 kahju:</td><td className="py-1 font-mono font-bold text-right text-indigo-800">{lisa3Total.toFixed(2)} €</td></tr>}
            <tr className="border-t-2 border-slate-900 font-black text-sm bg-slate-50">
              <td className="p-2.5 text-[10px] uppercase tracking-widest">KOKKU KESKKONNAKAHJU:</td>
              <td className="p-2.5 font-mono text-lg text-right">{combinedTotal.toFixed(2)} €</td>
            </tr>
          </tbody></table>
        </div>
      </div>
      ...
      {meta.markused && (
        <div className="mt-8 border-t border-slate-200 pt-4">
          <div className="text-[10px] uppercase font-bold text-slate-400 mb-1.5 tracking-wider">Märkused</div>
          <p className="text-xs text-slate-600 leading-relaxed whitespace-pre-wrap italic bg-slate-50 p-4 border rounded-xl">{meta.markused}</p>
        </div>
      )}

      <div className="mt-20 grid grid-cols-2 gap-12">
        <div className="border-t border-slate-300 pt-3 text-center text-[9px] text-slate-400 font-bold uppercase tracking-[0.2em]">Hinnangu koostaja allkiri / Digitempel</div>
        <div className="border-t border-slate-300 pt-3 text-center text-[9px] text-slate-400 font-bold uppercase tracking-[0.2em]">Allkirjastamise kuupäev</div>
      </div>
    </div>
  );
}
