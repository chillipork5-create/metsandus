  ...
  return (
    <div className="card overflow-hidden border border-slate-200 shadow-sm">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-4 bg-white hover:bg-slate-50 text-left">
        <div className="flex items-center gap-2"><Variable className="w-4 h-4 text-forest-500" /><span className="text-xs font-bold text-slate-700 uppercase tracking-wider">Lisa 3 kahjumäärade tabel</span></div>
      ...
      {isOpen && (
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
          ...
          <p className="mt-3 p-3 bg-forest-50/70 rounded-lg text-[10px] text-forest-800 border border-forest-100/50">Kahjumäär eurodes iga rinnaspindala (m²/ha) puudujääva osa kohta. Vanuse 0–20 ja 101+ korral kahju ei arvestata (—).</p>
        </div>
      )}
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <section className="card p-8">
            ...
            {lisa3.rows.length > 0 && (
              <div className="mt-4 p-4 bg-forest-50 border border-forest-100 rounded-xl flex justify-between items-center">
                <div><div className="text-[10px] font-bold text-forest-500 uppercase tracking-widest">Arvutatud G kokku</div><div className="text-lg font-bold text-forest-800">{calculatedG.toFixed(2)} m²/ha</div></div>
                <div className="text-[10px] text-forest-500 italic text-right">Korrigeeritud pindala: {pindala.toFixed(4)} ha</div>
              </div>
            )}
          </section>
          <section className="card p-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1"><label className="label-cap">Tegelik G pärast raiet</label>
                <input type="number" className={`input-field w-full ${lisa3.rows.length > 0 ? 'bg-forest-50 text-forest-800 font-bold border-forest-200' : ''}`}
                ...
                {lisa3.rows.length > 0 && <p className="text-[10px] text-forest-500 font-medium italic mt-1">G arvutatakse tabeli põhjal.</p>}
              </div>
          ...
          <section className="card p-8">
            ...
            <div className="mt-4 p-4 bg-slate-50 rounded-lg text-xs font-mono border border-slate-200">
              Korrigeeritud pindala: <span className="font-bold text-forest-600">{pindala.toFixed(4)} ha</span>
