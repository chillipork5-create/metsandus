  ...
  return (
    <div className="card overflow-hidden border border-slate-200 shadow-sm">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-4 bg-white hover:bg-slate-50 text-left">
        <div className="flex items-center gap-2"><Layers className="w-4 h-4 text-indigo-500" /><span className="text-xs font-bold text-slate-700 uppercase tracking-wider">Lisa 2 kahjumäärade tabel</span></div>
      ...
      {isOpen && (
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
          ...
          <div className="mt-3 p-3 bg-indigo-50/70 rounded-lg text-[10px] text-indigo-900 border border-indigo-100/50">
            ...
            <p className="mt-1.5 border-t border-indigo-100 pt-1 italic">Kännu läbimõõdul &gt; 102 cm lisandub iga 4 cm kohta: I: +{LISA2_STEP[0].toFixed(2)} €, II: +{LISA2_STEP[1].toFixed(2)} €, III: +{LISA2_STEP[2].toFixed(2)} €, IV: +{LISA2_STEP[3].toFixed(2)} €.</p>
          </div>
        </div>
      )}
    </div>
